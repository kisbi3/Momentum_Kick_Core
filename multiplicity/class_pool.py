...
class Fitting_gpu:
    ...
    def __Fr(self, xx, yy, zz, pt):
        # return xx+yy*pt*pt
        return xx*cp.exp(-yy/pt-zz*pt)
    def __Nk(self, A, B, multi):
        return cp.exp(-B/multi)
        # return cp.sqrt(B*multi)
    ...
    '''associated yield의 multiplicity 의존성을 fitting하기 위함.'''
    '''ATLAS와 CMS의 multiplicity가 완전히 다르기 때문에 main_class.py에서 따로 실행해야 함.'''
    def multi_fitting(self):
        '''여기에서는 phi array라고 언급되어 있지만, multiplicity를 대입하여 사용할 것임.'''
        popt, pcov = scipy.optimize.curve_fit(self.multi_fitting_func, xdata = self.phi_array, ydata = self.data, bounds=self.boundary, p0 = self.initial)
        return popt, np.sqrt(np.diag(pcov))


    def multi_fitting_func(self, given_array, xx, AA, BB):
        kick = 0.798958702
        Tem = 0.840820694
        yy = 1.49370324
        zz = 3.51939886e-12
        ...
        Aridge = 1/np.sum(cpu.Aridge(pti, yi, Tem, self.__m, self.__md, self.__a, self.__sqrSnn, self.__mp)*dyi*dpti*2*np.pi)

        # '''이거보다 더 좋은 방법이 있을거 같기는 한데 일단 pass'''
        # zero = np.zeros(len(given_array))
        # Aridge_zip = zero + Aridge;     xx_zip = zero + xx;     AA_zip = zero + AA;     BB_zip = zero + BB

        # pool = multiprocessing.Pool(processes=2)
        # result = pool.starmap(self.multiplicity_yield, zip(given_array, Aridge_zip, xx_zip, AA_zip, BB_zip))
        result = np.array([])
        for multi in given_array:
            result = np.append(result, self.multiplicity_yield(multi, Aridge, xx, AA, BB))
        self.__count = self.__count + 1
        print(f"{self.__count}회", kick, Tem, xx, yy, zz, AA, BB, np.sum((result-self.data)**2))
        return result

    def multiplicity_yield(self, multiplicity, Aridge, xx, AA, BB):
        Aridge = cp.asarray(Aridge); xx = cp.asarray(xx); AA = cp.asarray(AA); BB = cp.asarray(BB)
        kick = 0.798958702
        Tem = 0.840820694
        yy = 1.49370324
        zz = 3.51939886e-12
        bin = 300
        ptf, etaf, phif = cp.meshgrid(cp.linspace(self.ptf[0], self.ptf[1], bin), cp.linspace(self.etaf[0], self.etaf[1], bin), cp.linspace(self.__Yridge_phif_start, self.__Yridge_phif_end, bin))
        ...
        result = (4/3)*cp.sum(self.__Fr(xx, yy, zz, ptf)*self.__Nk(AA, BB, multiplicity)*ptf*gpu.Ridge_dist(Aridge, ptf, etaf, phif, kick, Tem, self.__sqrSnn, self.__mp, self.__m, self.__mb, self.__md, self.__a))*dptf*detaf*dphif/delta_Deltaeta
        return cp.asnumpy(result)


    #   fitting parameters : kick, Tem, xx, yy, AA, BB
    # def fitting_func__multi(self, given_array, kick, Tem, xx, yy, zz, AA, BB):
    '''이전에 fitting한 값(multiplicity X)을 고정시켜놓고 Nk만 바꿔가며 fitting'''
    '''xx는 전체적인 크기이므로 재 fitting'''
    '''AA는 현재 비활성화 상태. 이를 없애려면 전체적으로 수정이 필요하므로 그냥 방치.'''
    def fitting_func__multi(self, given_array, xx, AA, BB):
        kick = 0.798958702
        Tem = 0.840820694
        yy = 1.49370324
        zz = 3.51939886e-12
        ...
        for i in range(self.Number_of_Array):
            ...
            phi_array.append(given_array[start : start + number])
            start += number
        Aridge_bin = 500
        pti, yi = np.meshgrid(np.linspace(self.__pti[0], self.__pti[1], Aridge_bin), np.linspace(self.__yi[0], self.__yi[1], Aridge_bin))
        dyi = (self.__pti[1] - self.__pti[0])/Aridge_bin
        ...
        for i in range(len(self.multi)):
            dist = self.__multiplicity(self.multi[i], self.ptf[i], phi_array[i], self.etaf[i], Aridge, kick, Tem, xx, yy, zz, AA, BB)
    ...
    def __multiplicity(self, multi, ptf_range, phi_array, etaf, Aridge, kick, Tem, xx, yy, zz, A, B):
        bin = 300
        detaf = (etaf[1]-etaf[0])/bin
        delta_Deltaeta = 2*(etaf[1]-etaf[0])
        ptf, etaf, phif = cp.meshgrid(cp.linspace(ptf_range[0], ptf_range[1], bin), cp.linspace(etaf[0], etaf[1], bin), cp.asarray(phi_array))
        dptf = (ptf_range[1] - ptf_range[0])/bin
        # dist = cp.sum((4/3)*self.__Fr(xx, yy, zz, ptf)*self.__Nk(A, B, multi)*ptf*gpu.Ridge_dist(Aridge, ptf, etaf, phif, kick, Tem, self.__sqrSnn, self.__mp, self.__m, self.__mb, self.__md, self.__a), axis=(0))*dptf*detaf/delta_Deltaeta
        ...
        ridge_integrate = ridge_integrate*self.__Fr(xx, yy, zz, ptf)*self.__Nk(A, B, multi)
    ...
    # def fitting_func(self, kick, Tem, xx, yy):
        ...
        for i in range(len(phi_array)):
            ...
            if i==0:
                ...
                result = np.concatenate((result, result_dist[i]))
        result = np.concatenate((result, self.Yridge(Aridge, kick, Tem, xx, yy, zz)))       # fitting에 사용하는 데이터가 Yridge가 포함되어 있는 경우
        self.__count = self.__count + 1
    ...
    def __ptdep(self, phi_array, etaf, ptf_dist, Aridge, kick, Tem, xx, yy, zz):
        ...
        detaf = (etaf[1]-etaf[0])/bin
        delta_Deltaeta = 2*(etaf[1]-etaf[0])
        dist = cp.sum(self.__FrNk(xx, yy, zz, ptf)*ptf*gpu.Ridge_dist(Aridge, ptf, etaf, phif, kick, Tem, self.__sqrSnn, self.__mp, self.__m, self.__mb, self.__md, self.__a), axis=0)*dptf*detaf/delta_Deltaeta
    ...
    def Yridge(self, Aridge, kick, Tem, xx, yy, zz):
        ...
        for i in range(len(self.pt_range[0])):
            for j in range(len(self.pt_range[0][i])):
                ...
                result = (4/3)*cp.sum(self.__FrNk(xx, yy, zz, ptf)*ptf*gpu.Ridge_dist(Aridge, ptf, etaf, phif, kick, Tem, self.__sqrSnn, self.__mp, self.__m, self.__mb, self.__md, self.__a))*dptf*detaf/delta_Deltaeta
        ...
        for i in range(len(self.pt_range[0])):
            for j in range(len(self.pt_range[0][i])):
                ...
                dphif = (self.__Yridge_phif_end - self.__Yridge_phif_start)/bin
                delta_Deltaeta = 2*(etaf_range[i][1]-etaf_range[i][0])
                result = (4/3)*cp.sum(self.__FrNk(xx, yy, zz, ptf)*ptf*gpu.Ridge_dist(Aridge, ptf, etaf, phif, kick, Tem, self.__sqrSnn, self.__mp, self.__m, self.__mb, self.__md, self.__a))*dptf*detaf*dphif/delta_Deltaeta
...
class Drawing_Graphs:
    ...
    def __init__(self, etaf, kick, Tem, xx, yy, zz, AA, BB):
        # self.multi = multi
        # self.ptf = ptf
        self.etaf = etaf
        # self.phif = phif
    ...
    def __Ridge_Multi(self, multi, ptf_range, phif_range):
    ...
    def __Ridge_ptdep(self, ptf, phif_range):
        kick = self.kick; Tem = self.Tem; xx = self.xx; yy = self.yy; zz = self.zz
        Aridge = self.__Aridge()
        bin = 300
        delta_Deltaeta = 2*(self.etaf[1]-self.etaf[0])
        dptf = (ptf[1]-ptf[0])/bin
        ptf, etaf, phif = cp.meshgrid(cp.linspace(ptf[0], ptf[1], bin), cp.linspace(self.etaf[0], self.etaf[1], bin), cp.linspace(phif_range[0], phif_range[1], 100))
        detaf = (self.etaf[1]-self.etaf[0])/bin
        dist = (4/3)*cp.sum(self.__FrNk(xx, yy, zz, ptf)*ptf*gpu.Ridge_dist(Aridge, ptf, etaf, phif, kick, Tem, self.__sqrSnn, self.__mp, self.__m, self.__mb, self.__md, self.__a), axis = 1)*dptf*detaf/delta_Deltaeta

...
The assumption was already introduced in paper\cite{PbPb}, and the paper successfully explained the PbPb at 2.76 TeV results.
Hence, we need to check whether this assumption can also describe to proton-proton collisions.
...
However, when we fit the data via python package 'scipy', $p_T$ dependence terms go to zero.
Therefore, we think that proton-proton collisions do not have $p_T$ dpenedence at $f_R\langle N_k \rangle$, beacause they create small system.
...
% We fit the Momentum-Kick model as a least square method to the ALICE, CMS and ATLAS proton-proton collision 13TeV at high-multiplicity\cite{alice, cms, atlas}.
We fit the Momentum-Kick model as a least square method to the results of three major experimental colliders in the LHC proton-proton collisions 13TeV at high-multiplicity\cite{alice, cms, atlas}.
...
Experimental data is well described via the Momentum-Kick model for all $p_T$ ranges, except the CMS 13TeV $p_T$ distribution yield data.
...
We respectfully suggest that there may be inaccuracies in this data.
This is because the $\Delta \phi$ correlation results from the same center of mass energy are highly similar.
However, in the case of $p_T$ distribution, the data of different center of mass energy is similar.
Hence, we think that the CMS 13TeV $p_T$ distribution data is inaccurate.
...
However, due to the insufficient proton-proton data at 7 TeV, we have to find some parameters as physically at 7 TeV.
...
T_{7 \text{TeV}} = 1.25 \times \frac{1.226}{1.245} \approx 1.23
\end{equation}
...
\begin{table}[ht]
  ...
  \begin{tabular}{C{2cm} | C{3cm}  C{4cm}  C{2.5cm}  C{2.5cm}  C{2.5cm}}
   ...
   T (GeV) & 0.5 & 0.6 & 1.54 & 1.23 & 1.25\\[1ex]
   q (GeV) & 1.0 & 0.7 & 2 & 1.31 & 1.00\\ [1ex]
  $f_R \langle N_k \rangle$ & 4 & $20.2e^{-{1.395} / {\langle p_{T}^{\text{trig}} \rangle}-0.207{\langle p_{T}^{\text{trig}} \rangle}}$ & 0.93, 1.37 & 1.06 & 2.59\\[1.5ex]
   \hline
 \end{tabular}
...
Since the temperature has center of mass energy dependence, the temperature of proton-proton at 13 TeV is the highest of all the other experiments, which is $+108\%$ higher than PbPb at 2.76 TeV and $+150\%$ higher than AuAu at 200 GeV.
...
The average of momentum transfer per kick in proton-proton collisions at 13 TeV is about $43\%$ higher than PbPb at 2.76 TeV, but $24\%$ lower than pp at 7 TeV.
